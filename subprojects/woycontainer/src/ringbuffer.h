...
/// Similar to push but doesn't insert. It reuses whatever it's already present.
///
/// @param[out] out_item Pointer to the stored item.
/// @returns error
static int PRE(RinBuf_push_and_get) (PRE(RinBuf) *r, TYPE** out_item) {
    if (r->size == 0) return -1;
    r->head_pointer = (r->head_pointer + 1) % r->size;
    *out_item = &r->buffer[r->head_pointer];
    return 0;
}
...
static TYPE *PRE(RinBuf_get_relative) (const PRE(RinBuf) *r, size_t position) {
